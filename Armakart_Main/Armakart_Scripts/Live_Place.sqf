...
While {side player isEqualTo west} do 
{
	...
	if (player in (fullCrew [vehicle player, "driver", false] select 0)) then 
	{
		...
		Loc_Array = [Current_Laps, Current_Checkpoint, Next_Sub_CP_Num, _dist_calculated, player];

		[Loc_Array] remoteExec ["NJP_ADD_PLAYER_ARRAY_TO_PLACES_ARRAY", 0, true];
...
[[999, 999, 999, player]] remoteExec ["NJP_ADD_PLAYER_ARRAY_TO_PLACES_ARRAY", 0, true];