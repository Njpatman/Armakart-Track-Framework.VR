...
NJP_Powerup_Fnc_Miller_Lite = compileFinal preprocessFileLineNumbers "Armakart_Main\Armakart_Scripts\Armakart_Scripts_Powerups\Miller_Lite.sqf";
...
NJP_JIP_CHECK = {
  ...
  uiSleep 0.5;
	...
	if (RACE_PHASE isEqualTo "Customization_Phase") exitWith {
  ...
  if (RACE_PHASE isEqualTo "Voting_Phase") exitWith {
    ...
    call NJP_Client_Fnc_Voting_Phase_Edit;
...
NJP_Client_Fnc_Finish_Line = {
  ...
  if (Current_Laps + 1 > Laps_number && Current_Checkpoint isEqualTo CP_NUM) exitWith
  {
    ...
    if (player in (fullCrew [vehicle player, "cargo", false] select 0) && count units group player isEqualTo 2) then {
      ...
      [_Name, _Name_Dos] remoteExec ["NJP_PLACES_PUSHBACK", 0, true];
    ...
    if (player in (fullCrew [vehicle player, "driver", false] select 0) && count units group player isEqualTo 1) then {
      ...
      [_Name] remoteExec ["NJP_PLACES_PUSHBACK", 0, true];
