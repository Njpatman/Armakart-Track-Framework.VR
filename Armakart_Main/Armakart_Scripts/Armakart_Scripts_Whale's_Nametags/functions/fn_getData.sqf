		...
		//	For each member of the entity's crew (which would just be the entity, if it's a unit...)
		{
			if ( !(isNil "_x" || {_x isEqualTo _player}) ) then
			{
				...
				else
				{
					...
					// KARTS - CHANGED TO SPACE PROPERLY
					//_locationData =
					//{ ASLtoAGL (getPosASLVisual _x) vectorAdd [0,0,(1.4)] };
					_locationData = 
					{ ASLtoAGL (getPosASLVisual _x) vectorAdd [0,0.2,((1 + (((_player distance _x) * 1.5 * WHA_NAMETAGS_FONT_SPREAD_BOTTOM_MULTI))))] };
							// /_zoom
							
					// KARTS - ZOOM ADDS TOO MUCH SHAKE
